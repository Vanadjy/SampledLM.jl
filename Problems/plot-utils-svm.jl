...
function plot_svm(outstruct, sol, name="tr-qr")
    ...
    PGFPlots.save("svm-$(name).tikz", a, include_preamble=false)
    ...
    PGFPlots.save("svm-inner-outer-$(name).tikz", b, include_preamble=false)
    ...
    PGFPlots.save("svm-objdec-$(name).tikz", c, include_preamble=false)
    return objdec
end
