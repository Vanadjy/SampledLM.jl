...
function matrix_completion_model(xs, B, ω; sample_rate = 1.0)
  m, n = size(xs)

  #initializes sampling parameters
  sample = sort(randperm(size(B,1))[1:Int(sample_rate * size(B,1))])
  data_mem = copy(sample)
  res = vec(fill!(similar(xs[sample]), 0))

  function resid!(res, x; sample = sample)
    res .= 0
    res[ω] .= x[ω][sample] .- B[sample]
    res
  end
